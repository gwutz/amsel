...
static void amsel_memory_database_db_init (AlbDatabaseInterface *iface);
G_DEFINE_TYPE_WITH_CODE (AmselMemoryDatabase, amsel_memory_database, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (ALB_TYPE_DATABASE,
...
static void
amsel_memory_database_save_channel (AlbDatabase  *db,
                                    AlbChannel   *channel,
                                    GError        **error)
{
  ...
  if (self->set_error == TRUE) {
    g_set_error (error, ALB_DATABASE_ERROR, ALB_DATABASE_ERROR_INSERT, "Got an error");
  }
...
static GPtrArray *
amsel_memory_database_get_channels (AlbDatabase *db)
{
  return NULL;
}

static void
amsel_memory_database_db_init (AlbDatabaseInterface *iface)
{
